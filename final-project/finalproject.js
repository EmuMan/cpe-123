...
let sketch = function(p) {
   ...
   p.draw = function() {
      ...
      for (let i = 0; i < cloudCount; i++) {
         ...
         p.push();
            // lower layer
            let noiseVal = circularNoise(2, theta, p.frameCount / 500);
            p.translate(p.cos(theta) * cloudsRadius, cloudHeight + noiseVal * cloudSize / 2, p.sin(theta) * cloudsRadius);
            let r = Math.max((noiseVal - 0.3) * cloudSize, 0);
            if (r > 1.8) p.sphere(r, 5, 5);
         p.pop();
         p.push();
            // upper layer
            noiseVal = circularNoise(2, theta, p.frameCount / 500 + 50);
            p.translate(p.cos(theta) * cloudsRadius * 0.8, cloudHeight + 30 + noiseVal * cloudSize / 2, p.sin(theta) * cloudsRadius * 0.8);
            r = Math.max((circularNoise(2, theta, p.frameCount / 500 + 50) - 0.3) * cloudSize, 0);
            if (r > 2.0) p.sphere(r, 5, 5);
         p.pop();
      }
