...
class PlayerCamera extends P5Camera {
    ...
    constructor(name, location, pitch, yaw) {
        super(name, location, new p5.Vector(0, 0, -1), new p5.Vector(0, 1, 0));
        this.pitch = pitch;
        this.yaw = yaw;
    }
    ...
    update() {
        if (this.parent) this.location = this.parent.location; // probably only need to do this once? idk how references work here
        this.target = this.getForwardVector();
        this.target.add(this.location);
    }

    getForwardVector() {
        let vec = new p5.Vector(0, 1, 0);
        ...
        rotateVector(vec, new p5.Vector(0, 0, this.pitch));
        return vec;
    }
